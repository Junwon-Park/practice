...
const kakaoGeoAPI = async (adrs) => {
    ...
    if(res.data.documents.length === 0) { // 데이터의 결측치 예외 처리(Kakao Local API 요청 시, 주소에 대한 정보가 없어 빈 배열로 응답이 오는 경우)
...
const returnFinalData = async () => { // 아래 비동기 함수인 addGeoData와 kakaoGeoAPI를 호출해서 그 결과 값을 받아서 다음을 처리하기 위한 async 함수
    ...
    for(let i = 0; i < csvRow.length - 1; i++) { // Source Data의 주소지와 지번 부분을 결합해서 하나의 주소로 만들어서 한글 주소이기 때문에 encodeURI 메서드로 인코딩하여 대입한다.
        finalData.push(await addGeoData(kakaoGeoAPI(encodeURI(`${csvRow[i].split(',')[3]} ${csvRow[i].split(',')[4]}`)), csvRow[i])); // 데이터에 좌표 데이터 추가
    }
