...
export class CreateBoarDto {
  ...
  @IsNotEmpty()
  // 이런식으로 유효성 검사를 등록해 놓고 이 DTO를 사용하는 부분(Handler level, Parameter level, Global level)에 ValidationPipe를 등록하면 해당 유효성 검사를 수행한 뒤 검사를 모두 통과하면 값을 넘겨주고 통과하지 못하면 에러를 발생시킨다.
  // title이라는 속성에 값이 없는지 검사하는 유효성 검사를 등록해 놓은 것이다.
  title: string;
